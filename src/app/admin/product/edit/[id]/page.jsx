...
export default function EditProductForm({ params }) {
    ...
    const fetchCategories = async () => {
      try {
        const res = await axios.get(
          `${process.env.NEXT_PUBLIC_SERVER}/api/admin/category`,
  ...
  useEffect(() => {
    const fetchProductData = async () => {
      ...
      setIsLoading(true);
      try {
        const apiUrl = `${process.env.NEXT_PUBLIC_SERVER}/api/admin/view-product-by-id/${id}`;
        ...
        setFormData({
          ...
          tags: Array.isArray(productData.tags)
            ...
            : productData.tags || "",
          category: productData.categoryId?._id || productData.categoryId || "",
          ...
          categoryAttributes: productData.attributes || [],
          returnPolicy: {
            isReturnable: productData.returnPolicy?.isReturnable ?? true,
            isReturnDays: productData.returnPolicy?.isReturnDays || 7,
            isReturnCost: productData.returnPolicy?.isReturnCost || 0,
          },
          warranty: {
            description: productData.warranty?.description || "",
            warrantyType: productData.warranty?.warrantyType || "Platform",
            policy: productData.warranty?.policy || "",
          },
          shippingDetails: {
            weight: productData.shippingDetails?.weight || 0,
            weightUnit: productData.shippingDetails?.weightUnit || "kg",
            height: productData.shippingDetails?.height || 0,
            width: productData.shippingDetails?.width || 0,
            depth: productData.shippingDetails?.depth || 0,
            dimensionUnit: productData.shippingDetails?.dimensionUnit || "cm",
            shippingOption: productData.shippingDetails?.shippingOption || [
              {
                shippingType: "Normal",
                cost: 0,
                estimatedDays: 3,
          ...
          productDimension: {
            weight: productData.productDimension?.weight || 0,
            weightUnit: productData.productDimension?.weightUnit || "kg",
            height: productData.productDimension?.height || 0,
            width: productData.productDimension?.width || 0,
            depth: productData.productDimension?.depth || 0,
            dimensionUnit: productData.productDimension?.dimensionUnit || "cm",
          },
        });
        ...
        if (productData.variants && Array.isArray(productData.variants)) {
          productData.variants.forEach((variant, index) => {
            if (variant.images && Array.isArray(variant.images)) {
              existingVariantImages[index] = variant.images;
            }
          });
        }
        ...
        setExistingImages({
          ...
          variantImages: existingVariantImages,
        });

        // Set category path if category exists
        if (productData.categoryId && categories.length > 0) {
          const findCategoryPath = (categoryId) => {
            ...
            if (category) {
              const path = [category];
              // Add parent categories to path if needed
              let current = category;
              while (current.parentId) {
                const parent = categories.find(
                  (c) => c._id === current.parentId
                );
                if (parent) {
                  path.unshift(parent);
                  current = parent;
    ...
    fetchProductData();
  }, [id, categories]); // Depend on categories to set category path
  ...
  const handleSubmit = async (e) => {
    ...
    try {
        ...
        shippingOption: [
          {
            ...
            cost:
              Number(
                productDataToSend.shippingDetails.shippingOption[0].cost
              ) || 0,
            estimatedDays:
              Number(
                productDataToSend.shippingDetails.shippingOption[0]
      ...
      const productDimension = {
        ...productDataToSend.productDimension,
        weight: Number(productDataToSend.productDimension.weight) || 0,
        height: Number(productDataToSend.productDimension.height) || 0,
        width: Number(productDataToSend.productDimension.width) || 0,
        depth: Number(productDataToSend.productDimension.depth) || 0,
      };
      ...
      console.log("📦 Product data being sent for edit:", productData);

      // Handle main product image only if a new one is selected
      if (mainImage) {
        console.log("🖼️ Appending new main image:", {
          name: mainImage.name,
          type: mainImage.type,
          size: mainImage.size,
        });
      ...
      if (variantImages && Object.keys(variantImages).length > 0) {
        Object.entries(variantImages).forEach(([variantIndex, imageFiles]) => {
          // Check if imageFiles is an array (multiple images) or single image
          const imageArray = Array.isArray(imageFiles)
            ? imageFiles
            : [imageFiles];

          imageArray.forEach((imageFile, imageIndex) => {
            if (imageFile) {
      ...
      // Use PATCH method for editing existing product
      const response = await axios.patch(
        `${process.env.NEXT_PUBLIC_SERVER}/api/admin/edit-product/${id}`,
        formDataObj,
        {
          withCredentials: true,
          headers: {
      ...
      // Redirect back to products list after successful edit
      setTimeout(() => {
  ...
  const handleVariantImageChange = (e, index) => {
    if (e.target.files && e.target.files[0]) {
      ...
      if (updatedVariants[index]) {
        updatedVariants[index].hasImage = true;
        setFormData((prev) => ({
          ...prev,
  ...
  const handleSubmitOld = async (e) => {
    ...
    try {
      console.log(
        "API URL:",
        `${process.env.NEXT_PUBLIC_SERVER}/api/admin/add-product`
      );
      ...
      <div className="flex items-center justify-center min-h-screen">
        <div className="text-center max-w-md mx-auto p-6">
          <div className="bg-red-50 border border-red-200 text-red-700 px-4 py-3 rounded mb-4">
            <h2 className="font-bold text-lg mb-2">Failed to Load Product</h2>
            <p>{submitError}</p>
          </div>
          <Button
  ...
  return (
    <div className="flex flex-col lg:flex-row gap-8 h-full w-[70vw] mx-auto">
        ...
        <Tabs defaultValue="basicInfo" className="w-full ">
            ...
            <div className="relative inline-block w-full">
              ...
              {categoryMenuOpen && (
                ...
                >
                  <CascadingMenu
                    categories={categories}
                    onSelect={async (cat, path) => {
                      setSelectedCategoryPath(path);
                      setCategoryMenuOpen(false);
                      // Debug: log selected category
                      console.log("Selected category:", cat);
                      // Try to use handleSelectChange to set attributes
                      if (cat && cat._id) {
                        await handleSelectChange("category", cat._id);
                        // After handleSelectChange, log the updated formData
                        setTimeout(() => {
                          console.log(
                            "formData after category select:",
                            formData
                          );
                        }, 100);
                        // If attributes are missing, try fetching by ID
                        if (
                          !cat.attributes ||
                          !Array.isArray(cat.attributes) ||
                          ...
                          try {
                            const res = await axios.get(
                              `${process.env.NEXT_PUBLIC_SERVER.replace(
                                "/api",
                                ""
                              )}/api/admin/category/${cat._id}`,
                              { withCredentials: true }
                            );
                            const fetchedCat =
                              res.data?.data?.data || res.data?.data;
                            console.log("Fetched category by ID:", fetchedCat);
                            if (
                              fetchedCat &&
                              Array.isArray(fetchedCat.attributes)
                            ) {
                            ...
                            console.error(
                              "Failed to fetch category by ID for attributes",
                              err
                            );
                          }
                        }
                  ...
                  <div key={index} className="p-4 border rounded-md mt-4">
                    ...
                    <div>
                      ...
                      >
                        ...
                        {existingImages.variantImages[index]?.length > 0 &&
                          !variantImages[index] && (
                            <div className="w-full mt-4">
                              ...
                              <div className="flex gap-2 flex-wrap">
                                {existingImages.variantImages[index].map(
                                  (imageUrl, imgIdx) => (
                                    <div
                                      key={imgIdx}
                                    ...
                                    >
                                      <img
                                        src={imageUrl}
                                        alt={`Current Variant ${index} - Image ${imgIdx}`}
          ...
          <TabsContent value="images" className="space-y-4">
            ...
            <div className="w-full">
              ...
              {existingImages.mainImage && !mainImage && (
                <div className="mt-4">
                  <Label className="text-sm text-gray-600">
                    Current Main Image
                  </Label>
                  <img
                    src={existingImages.mainImage}
      ...
      <div className="w-full lg:w-1/3 p-6 border rounded-lg h-fit bg-white shadow">
        ...
        <ProductPreview
          formData={formData}
          mainImage={mainImage || existingImages.mainImage}
