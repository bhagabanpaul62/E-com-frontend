...
export default function ProductAnalytics() {
    ...
    const fetchProducts = async () => {
      ...
      try {
        const res = await axios.get(
          `${process.env.NEXT_PUBLIC_SERVER}/api/admin/view-products`,
  ...
  const processAnalytics = (productsData) => {
    ...
    const priceRanges = [
      {
        range: "$0-$50",
        count: productsData.filter((p) => p.price >= 0 && p.price <= 50).length,
      },
      {
        range: "$51-$100",
        count: productsData.filter((p) => p.price > 50 && p.price <= 100)
          .length,
      },
      {
        range: "$101-$200",
        count: productsData.filter((p) => p.price > 100 && p.price <= 200)
          .length,
      },
      {
        range: "$201-$500",
        count: productsData.filter((p) => p.price > 200 && p.price <= 500)
          .length,
      },
      {
        range: "$500+",
        count: productsData.filter((p) => p.price > 500).length,
      },
    ];
    ...
    const stockDistribution = {
      ...
      "Healthy Stock (11-50)": productsData.filter(
        (p) => p.totalStock >= 11 && p.totalStock <= 50
      ).length,
      ...
      <div className="flex justify-between items-center mb-8">
        <div>
          ...
          <p className="text-gray-500">
            Insights and statistics about your product inventory
          </p>
        </div>
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-gray-400">
                {analytics.totalProducts > 0
                  ? `${Math.round(
                      (analytics.outOfStock / analytics.totalProducts) * 100
                    )}% of total`
                  : "0% of total"}
              </p>
            </div>
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-gray-400">
                {analytics.totalProducts > 0
                  ? `${Math.round(
                      (analytics.lowStock / analytics.totalProducts) * 100
                    )}% of total`
                  : "0% of total"}
              </p>
            </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
              ...
              <div key={range.range}>
                ...
                <div className="w-full bg-gray-200 rounded-full h-2.5">
                  <div
                    ...
                    style={{
                      width: `${
                        (range.count / analytics.totalProducts) * 100
                      }%`,
                    }}
                  ></div>
                </div>
        ...
        <div className="bg-white rounded-lg shadow p-6 border border-gray-100">
          ...
          <div className="grid grid-cols-2 gap-4">
            {Object.entries(analytics.stockDistribution).map(
              ([label, value]) => (
                <div
                  key={label}
                ...
                <div
                  ...
                  style={{
                    height: `${
                      (item.count /
                        Math.max(
                          ...analytics.monthlyTrends.map((i) => i.count)
                        )) *
                      200
                    }px`,
                  }}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          <div className="border border-gray-200 rounded-lg p-4">
            ...
            <div className="flex items-center">
              ...
              <span className="text-sm font-medium">
                {Math.round(
                  100 - (analytics.outOfStock / analytics.totalProducts) * 100
                )}
                %
              </span>
            </div>
          ...
          <div className="border border-gray-200 rounded-lg p-4">
            ...
            <p className="text-sm text-gray-600">
              Your products are spread across {analytics.categories.length}{" "}
              categories.
              {analytics.topCategories.length > 0 &&
                ` ${analytics.topCategories[0].name} is your largest category.`}
            </p>
          </div>
          ...
          <div className="border border-gray-200 rounded-lg p-4">
            ...
            <p className="text-sm text-gray-600">
              {
                analytics.priceRanges.sort((a, b) => b.count - a.count)[0]
                  ?.range
              }{" "}
              is your most common price range.
            </p>
          </div>
        </div>
      </div>
