...
export function ProductPreview({
  ...
  // Function to get the correct image src
  const getImageSrc = () => {
    if (mainImage) {
      // If mainImage is a File object (new upload)
      if (mainImage instanceof File) {
        return URL.createObjectURL(mainImage);
      }
      // If mainImage is a URL string (existing image)
      return mainImage;
    }
    return null;
  };
  ...
  return (
    <div className="space-y-2">
      ...
      {variantImages && Object.keys(variantImages).length > 0 && (
        <div className="space-y-2">
          <p className="text-sm font-medium text-gray-600">Variant Images:</p>
          <div className="grid grid-cols-3 gap-1">
            {Object.entries(variantImages).map(([variantIndex, images]) => {
              const imageArray = Array.isArray(images) ? images : [images];
              return imageArray.map((image, imgIdx) => {
                if (image) {
                  const imageSrc =
                    image instanceof File ? URL.createObjectURL(image) : image;
                  return (
                    <div
                      key={`${variantIndex}-${imgIdx}`}
                      className="w-16 h-16"
                    >
                      <img
                        src={imageSrc}
                        alt={`Variant ${variantIndex} - ${imgIdx}`}
