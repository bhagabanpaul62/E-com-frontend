// Moved from user/categoryNavigation.jsx
"use client";
...
export default function CategoryNavigation() {
  ...
  function buildCategoryTree(categories, parentId = null) {
    const filteredCategories = categories.filter((cat) => {
      ...
      if (cat.parentId && parentId !== null) {
        ...
        console.log("our sub categor", cat.parentId);
      }
      return false;
    });
  ...
  const renderFlipkartMenu = (cats) => {
    return (
      <div className="bg-white border-b hidden  lg:block border-gray-200 shadow-sm">
                ...
                >
                  <div className="flex items-center space-x-1 py-3 px-2 cursor-pointer group-hover:text-blue-600 transition-colors">
                    <Link
                      ...
                      className="font-medium text-sm text-gray-700 group-hover:text-blue-600"
                    >
                      {cat.name}
                    </Link>
                                ...
                                <div key={subCat._id} className="group/sub">
                                  <div className="flex items-center justify-between py-2 px-3 rounded-lg hover:bg-white hover:shadow-sm transition-all cursor-pointer">
                                    <Link
                                      ...
                                      className="font-medium text-gray-700 group-hover/sub:text-blue-600"
                                    >
                                      {subCat.name}
                                    </Link>
                                  ...
                                  <div className="p-3 rounded-lg hover:bg-blue-50 cursor-pointer transition-colors">
                                    ...
                                    <div className="space-y-1">
                                      {subCat.Children.slice(0, 4).map(
                                        (subSubCat) => (
                                          <Link
                                            href={`/category/${subSubCat._id}`}
                                            key={subSubCat._id}
                                            className="text-sm text-gray-600 hover:text-blue-600 cursor-pointer block"
                                          >
                                            {subSubCat.name}
                                          </Link>
                                        )
              ...
              <div className="flex items-center space-x-3 py-3 overflow-x-auto scrollbar-hide">
                {category.slice(0, 12).map((cat) => (
                  <div key={cat._id} className="flex-shrink-0">
                    <Link
                      href={`/category/${cat._id}`}
                      className="inline-block px-3 py-2 text-xs font-medium text-gray-700 bg-gray-100 rounded-full whitespace-nowrap hover:bg-blue-100 hover:text-blue-600 transition-colors cursor-pointer"
                    >
                      {cat.name}
                    </Link>
                  </div>
